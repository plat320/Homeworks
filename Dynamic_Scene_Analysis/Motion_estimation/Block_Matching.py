...
parser.add_argument('-t', '--search_type', type=str, default="three", help='select type of searching algorithm Full | Three')
...
if __name__ == '__main__':
    ...
    img_dir = "./example"
    ref_img_name = "cubecut1.tif"
    target_img_name = "cubecut2.tif"

    ref_image = cv2.imread(os.path.join(img_dir, ref_img_name),cv2.IMREAD_GRAYSCALE)
    target_image = cv2.imread(os.path.join(img_dir, target_img_name),cv2.IMREAD_GRAYSCALE)
    ...
    #### get PSNR
    PSNR = 20 * np.log10(255 / np.sqrt(l2(recon_image, target_image * 255)))

    cv2.imwrite("reconstruction_image_-BS_SR_type_PSNR-{}_{}_{}_{:.2f}.png".format(args.block_size, args.search_range, args.search_type, PSNR), ref_image)
